...
public class FinanceChart extends MultiChart {
    ...
    public String getToolTipDateFormat() {
        ...
        double resolution = (m_timeStamps[m_timeStamps.length - 1] - m_timeStamps[0]) / (m_timeStamps.length);
    ...
    private void setXLabels(Axis a) {
        ...
        if (m_extraPoints < m_timeStamps.length) {
            int tickStep = (int) ((m_timeStamps.length - m_extraPoints) * m_timeLabelSpacing / (m_totalWidth - m_leftMargin - m_rightMargin)) + 1;
            double timeRangeInSeconds = m_timeStamps[m_timeStamps.length - 1] - m_timeStamps[m_extraPoints];
    ...
    //////////////////////////////////////////////////////////////////////////////
    // Create tool tip format string for showing OHLC(Open, High, Low, Close) data
    //////////////////////////////////////////////////////////////////////////////
    private String getHLOCToolTipFormat() {
        return "title='" + getToolTipDateFormat() + "\r\n" + " Open:{open|" + m_generalFormat + "}\r\n, High:{high|"
                + m_generalFormat + "}\r\n, Low:{low|" + m_generalFormat + "}\r\n, Close:{close|" + m_generalFormat
                + "}'";
...
//        ret.setHTMLImageMap("", "", getHLOCToolTipFormat());
        ...
        if (m_highData.length > m_extraPoints) {
            int expectedWidth = (int) ((m_totalWidth - m_leftMargin - m_rightMargin) / (m_highData.length - m_extraPoints));
            if (expectedWidth <= 5) {
...
//        ret.setHTMLImageMap("", "", getHLOCToolTipFormat());
    ...
    private void addOHLCLabel(int upColor, int downColor, boolean candleStickMode) {
        ...
        if (i >= 0) {
            ...
            if (openValue != Chart.NoValue) {
                openLabel = "Open:" + formatValue(openValue, m_generalFormat);
                delim = ", ";
            }
            if (highValue != Chart.NoValue) {
                highLabel = delim + "High:" + formatValue(highValue, m_generalFormat);
                delim = ", ";
            }
            if (lowValue != Chart.NoValue) {
                lowLabel = delim + "Low:" + formatValue(lowValue, m_generalFormat);
                delim = ", ";
            }
            if (closeValue != Chart.NoValue) {
                closeLabel = delim + "Close:" + formatValue(closeValue, m_generalFormat);
                delim = ", ";
            }
    ...
    private BarLayer addVolBars2(XYChart c, int height, int upColor, int downColor, int flatColor) {
        ...
        DataSet dnDS = barLayer.addDataSet(new ArrayMath(m_volData).selectLTZ(closeChange).result(),
                downColor);
        DataSet flatDS = barLayer.addDataSet(new ArrayMath(m_volData).selectEQZ(closeChange).result(), flatColor);
    ...
    /// <param name="name">The name of the indicator.</param>
    /// <returns>The LineLayer object representing the line created.</returns>
    public void addZone(XYChart c, double x1, double x2, int color) {
        c.xAxis().addZone(x1, x2, color);
    }

    public void addTrackedZone(double x1, double x2, int color) {
        addZone(m_mainChart, x1, x2, color);
    ...
    public BarLayer addBarIndicator3(XYChart c, double[] data, int color, String name) {
        double[] down = new double[data.length];
        ...
        BarLayer layer = c.addBarLayer(up, 0xff3333, formatIndicatorLabel(name, data));
        c.addBarLayer(down, 0x33ff33, formatIndicatorLabel(name, data));
        layer.setBorderColor(Chart.Transparent);
        return layer;
    }

    public SplineLayer addSplineLayer(XYChart c, double[] data) {
    ...
    public XYChart addChaikinVolatility(int height, int period1, int period2, int color) {
        ...
        return addLineIndicator(height, new ArrayMath(m_highData).sub(m_lowData).expAvg(2.0 / (period1 + 1)).rate(period2).sub(1).mul(100).result(), color, label);
    }
...
//        addBarIndicator2(c, new ArrayMath(macd).sub(macdSignal).result(), divColor, "Divergence");
        addBarIndicator3(c, new ArrayMath(macd).sub(macdSignal).result(), divColor, "Divergence");
    ...
    public XYChart getM_mainChart() {
        return m_mainChart;
    }

    public void setM_mainChart(XYChart m_mainChart) {
        this.m_mainChart = m_mainChart;
    }
